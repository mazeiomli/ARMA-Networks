...
def main():
    ...
    test_data = dataset.SemData(split='test', root=args.data_root, data_list=args.test_list, transform=test_transform)
    index_start = args.index_start
    if args.index_step == 0:
        index_end = len(test_data.image_paths)
    else:
        index_end = min(index_start + args.index_step, len(test_data.image_paths))
    test_data.data_list = test_data.image_paths[index_start:index_end]
    ...
    if not args.has_prediction:
        if args.arch == 'psp':
            ...
            model = PSPNet(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor, pretrained=False, arma=args.arma)
        elif args.arch == 'psa':
...
def test(test_loader, data_list, model, classes, mean, std, base_size, crop_h, crop_w, scales, gray_folder, color_folder, colors):
    ...
    JS = JS/length
    DC = DC/length
    print(num_params,acc,SE,SP,PC,F1,JS)
    check_makedirs(gray_folder)
    f = open(os.path.join(gray_folder,'result.csv'), 'a', encoding='utf-8', newline='')
    wr = csv.writer(f)
    wr.writerow(["num_params","Acc","Sens","Spec","Prec","F1","IoU"])
    wr.writerow([num_params,acc,SE,SP,PC,F1,JS])
    f.close()
        # check_makedirs(gray_folder)

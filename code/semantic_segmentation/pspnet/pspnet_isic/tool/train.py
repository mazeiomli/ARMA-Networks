...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'psp':
        ...
        model = PSPNet(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor, criterion=criterion, arma=args.arma, pretrained=False)
    ...
    train_data = dataset.SemData(split='train', root=args.data_root, data_list=args.train_list, transform=train_transform)
    if args.distributed:
    ...
    if args.evaluate:
        val_transform = transform.Compose([
            ...
            transform.ToTensor(),
            transform.Normalize(mean=mean, std=std)])
        val_data = dataset.SemData(split='val', root=args.data_root, data_list=args.val_list, transform=val_transform)
        if args.distributed:

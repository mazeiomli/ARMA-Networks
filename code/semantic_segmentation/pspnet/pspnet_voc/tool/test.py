...
def test(test_loader, data_list, model, classes, mean, std, base_size, crop_h, crop_w, scales, gray_folder, color_folder, colors):
    ...
    for i, (input, labels) in enumerate(test_loader):
        ...
        metrics.update(labels.numpy(), np.expand_dims(prediction,0))
        ...
        os.makedirs(f'{gray_folder}/results/', exist_ok=True)
    ...
    print(score)
    # f = open(os.path.join(f'{gray_folder}/results/','result.csv'), 'a', encoding='utf-8', newline='')
    # wr = csv.writer(f)
    # wr.writerow(score)
    # f.close()
